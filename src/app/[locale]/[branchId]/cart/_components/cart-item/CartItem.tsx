...
const CartItem = ({ item, onIncrease, onDecrease, onRemove, unavailableReason }: CartItemProps) => {
    const dimmed = Boolean(unavailableReason);
    return (
        <div
            className={`group flex gap-4 bg-white p-3 rounded-2xl border border-gray-100 relative overflow-hidden ${
                dimmed ? "opacity-60" : ""
            }`}
        >
            ...
            <div className="flex-1 flex flex-col justify-between py-1">
                <div>
                    ...
                    {unavailableReason && (
                        <p className="text-[11px] text-red-500 mt-1">{unavailableReason}</p>
                    )}
                </div>

                <div className="flex justify-between items-end mt-2">
                    <Price price={item.price * item.qty} size="base" color="default" />
